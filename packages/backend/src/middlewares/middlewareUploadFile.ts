...
const sanitizeInput = (req: Request, res: Response, next: NextFunction) => {
  const sanitizeString = (str?: string): string => {
    if (typeof str !== 'string') return ''; // החזרת מיתר ריק אם הקלט לא מיתר
    return str.replace(/[\u0000-\u001F\u007F-\uFFFF]/g, ''); // הסרת תווים לא נתמכים
  };
  ...
  if (req.body.tags && Array.isArray(req.body.tags)) { // בדיקה אם tags הוא מערך
    req.body.tags = req.body.tags
      .map((tag: string) => sanitizeString(tag)) // סניטיזציה של כל תג
      .filter((tag: string) => tag.length > 0); // סינון תגיות ריקות
  } else {
    req.body.tags = []; // הגדר למערך ריק אם לא מערך
  }
